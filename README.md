...
> 📄 **실험 보고서: [REPORT.md](REPORT.md)** — 접근 방법·전략, 무엇이 통했고 무엇이 실패했는지, 검증 방법론을 기재했습니다.
...
학습·조립 쪽은 torch 2.8 / transformers 4.x 에서 검증돼, 한 환경에 담을 수 없었습니다.
어떤 스크립트를 돌리느냐에 따라 설치할 파일이 다릅니다 (버전·요건은 [환경](#환경) 절 참고):

| 하려는 것 | 환경 | 설치 |
|---|---|---|
| 제출 CSV 조립·검증 (`compose_final_submissions.py`, `reproduce_all.sh compose`), 학습 (`train_*.py`), 데이터 준비 | 시스템 `python3` | `requirements.txt` |
...
### 테스트 공개 이후 커밋 (2026-08-31, 재현성·문서 한정)
...
logs/          GRPO 학습 로그
REPORT.md      실험 보고서 (Experiment Report) — 접근·전략·검증 방법론
```
